...
client.once("ready", async () => {
  // Start of Bot Status
  client.user.setActivity("court cases and people's abuses.", { type: ActivityType.Listening });
  //End of bot status
  ...
  await client.guilds.cache.get("989558770801737778").commands.set(staffCommands); // guild commands for FS Automations (includes the rest of the commands)
  await client.guilds.cache.get("984205067475374141").commands.set(staffCommands); // this should be for FSS, suman add the FSS guild id here
  ready = true;
...
process.emitWarning = (warning, type) => {
  if (type === 'DeprecationWarning' && warning.includes('punycode')) return;
  console.warn(warning);
};
